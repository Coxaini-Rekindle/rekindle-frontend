import type { PostDto } from "@/types/memory";
...
interface MemoryReactionBarProps {
  mainPost: PostDto;
  onReactionClick: (reactionType: ReactionTypeDto) => void;
}

export default function MemoryReactionBar({
  mainPost,
  onReactionClick,
}: MemoryReactionBarProps) {
  ...
  const usedReactions = Object.entries(mainPost.reactionSummary.reactionCounts)
    .filter(([_, count]) => count > 0)
    .map(([type, count]) => ({
      type: type as ReactionTypeDto,
      count,
      emoji: getReactionEmoji(type as ReactionTypeDto),
      isUserReaction: mainPost.reactionSummary.userReactions.includes(
